...
// ActivateUser implements AuthRepository.
func (a *authRepo) ActivateUser(email string) error {
	c, cancel := context.WithCancel(context.Background())
	defer cancel()
	query := "UPDATE users SET verified = true WHERE email = $1"
	_, err := a.db.ExecContext(c, query, email)
	if err != nil {
...
func (a *authRepo) CreateUser(user *models.UserReq) (*int, error) {
	...
	defer cancel()
	var userId int
	query := "INSERT INTO users(fullname,email,password,verified) VALUES($1,$2,$3,$4)  RETURNING id"
	// _, err := a.db.ExecContext(c, query, user.FullName, user.Email, user.Password)
	err := a.db.QueryRowContext(c, query, user.FullName, user.Email, user.Password, false).Scan(&userId)
